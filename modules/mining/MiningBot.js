...
class Bot extends ModuleBase {
    ...
    renderNormal() {
        ...
        if (this.lastAimPos) {
            ...
            Render3D.drawLine(new Vec3d(x - d, y, z), new Vec3d(x + d, y, z), palette.aimColor, 2, true);
            Render3D.drawLine(new Vec3d(x, y - d, z), new Vec3d(x, y + d, z), palette.aimColor, 2, true);
            Render3D.drawLine(new Vec3d(x, y, z - d), new Vec3d(x, y, z + d), palette.aimColor, 2, true);
        }

        if (this.lastNextPos) {
            Render3D.drawStyledBox(new Vec3d(this.lastNextPos.x, this.lastNextPos.y, this.lastNextPos.z), palette.nextFill, palette.nextWire, 6, true);
        }
    }
    ...
    renderDebug() {
        if (this.foundLocations.length > 0) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                Render3D.drawWireFrameBox(new Vec3d(loc.x, loc.y, loc.z), new RenderColor(r * 255, g * 255, b * 255, 255), 5, true);

                if (loc.aimX !== undefined) {
                    const d = 0.1;
                    const color = new RenderColor(r * 255, g * 255, b * 255, 230);
                    Render3D.drawLine(new Vec3d(loc.aimX - d, loc.aimY, loc.aimZ), new Vec3d(loc.aimX + d, loc.aimY, loc.aimZ), color, 3, true);
                    Render3D.drawLine(new Vec3d(loc.aimX, loc.aimY - d, loc.aimZ), new Vec3d(loc.aimX, loc.aimY + d, loc.aimZ), color, 3, true);
                    Render3D.drawLine(new Vec3d(loc.aimX, loc.aimY, loc.aimZ - d), new Vec3d(loc.aimX, loc.aimY, loc.aimZ + d), color, 3, true);
                }
            }
        }
    }
